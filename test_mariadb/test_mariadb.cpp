...
int main(int argc, char* argv[])
{
    ...
    /* -----------------------------
       ...
       /* -----------------------------
          ...
          -----------------------------*/
          /* 查询命令：
              select stu_sex, stu_no, stu_name, stu_class_fname from view_student_for_oop  where stu_sex = "女" and stu_class_sname = "" limit 3
              含义：表示查询student表的性别为"女"并且专业简称为""的记录，每条记录要学号、姓名、年龄、专业全称四个字段且桉此顺序排序
              返回值：成功返回0，不成功非0
              不成功的常见可能性：
                1、查询字符串存在语法错误
                2、查询不存在的数据表 */
    if (mysql_query(mysql, "select stu_sex, stu_no, stu_name, stu_class_fname from view_student_for_oop  where stu_sex = \"女\" and stu_class_sname = \"\" limit 3")) {
        cout << "mysql_query failed(" << mysql_error(mysql) << ")" << endl;
        return -1;
    }
