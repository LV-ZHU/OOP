     ...
     2、本例中，cpp和不在同一目录下，则需要用相对路径方式来包含
        目录结构如下：
          BigHW
            ...
            |       cmd_console_tools.h
        说明：
          a) test-cct和include是兄弟关系，父目录都是BigHW
          b) 在操作系统中，
             ".."表示父目录，例如："../include"表示父目录下的include子目录
             "." 表示自己，  例如："./test-cct.cpp"和"test-cct.cpp"等价，都表示当前目录下的test-cct.cpp文件
          c) test-cct中的cpp想包含include中的h，则相对test-cct来说，h在test-cct的父亲BigHW的include子目录
...
int main()
{
	...
	if (1) {
		cct_setcursor(CCT_CURSOR_VISIBLE_FULL);
		to_be_continued("光标显示为全高色块", 0, 2);

		cct_setcursor(CCT_CURSOR_VISIBLE_HALF);
	...
	if (1) {
		...
		cct_setcursor(CCT_CURSOR_VISIBLE_NORMAL);	//打开光标
