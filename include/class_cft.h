...
private:
	...
	group_node* cfg_list;//需要在is_read_succeeded()判断是否成功建立cfg_list链表的指针
	void release_memory();//释放链表内存，异常或者析构的时候调用
	void parse_file();//解析配置文件，建立链表
	group_node* find_group(const string& group_name, bool is_case_sensitive) const;
	item_node* find_item(group_node* group, const string& item_name, bool is_case_sensitive) const;
